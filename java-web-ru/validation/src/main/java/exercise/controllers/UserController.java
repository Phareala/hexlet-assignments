...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(email -> EmailValidator.getInstance().isValid(email), "Поле email должно быть корректным");
        Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
                .check(pass -> StringUtils.isNumeric(pass), "Пароль должен состоять только из цифр")
                .check(pass -> pass.length() > 4, "Пароль должен быть больше 4 цифр");
